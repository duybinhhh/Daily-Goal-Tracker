...
## 12. Phòng Kỷ Luật (Discipline Room) - AI Camera Coach thật

Phòng Kỷ Luật (Discipline Room) là tính năng mô phỏng không gian học tập và làm việc chung, ứng dụng trí tuệ nhân tạo **AI Face Detection** (MediaPipe) để giám sát và đánh giá sự tập trung của bạn trong thời gian thực ngay trên trình duyệt.
...
*   **Tạo phòng (Create Room):**
    ...
    5.  **Công khai phòng (Public Room) [NEW]**: Bạn có thể bật tùy chọn "Công khai phòng" để phòng của mình xuất hiện trong danh sách chờ chung, giúp người dùng khác có thể tìm thấy và tham gia cùng bạn mà không cần mã mời. Nếu tắt, phòng sẽ là riêng tư và chỉ những ai có mã mời mới có thể vào.
    6.  Nhấn nút **Tạo phòng**. Lúc này hệ thống sẽ tự động khởi tạo bộ não AI (WASM Runtime).
*   **Danh sách phòng chờ công khai (Public Waiting List) [NEW]:**
    *   Tại màn hình khởi tạo của Discipline Room, cuộn xuống dưới để thấy danh sách các phòng đang chờ partner.
    *   Bạn có thể sử dụng bộ lọc để tìm phòng theo Chế độ tập trung (Study/Deep Work) hoặc Thời lượng (5, 15, 25 phút).
    *   Mỗi thẻ phòng hiển thị thông tin về người tạo (Tên, Level), tên phòng, chế độ và thời lượng.
    *   Nhấn **Tham gia** để vào ngay phòng đó mà không cần nhập mã mời.
*   **Phòng chuẩn bị & Trao đổi mục tiêu (Ready Room - Pre-Session Lobby) [NEW]:**
    *   Khi cả hai người cùng vào phòng, phiên tập trung chính thức và camera AI chưa khởi chạy ngay. Bạn sẽ vào màn hình **Lobby**.
    *   **Đặt mục tiêu phiên**: Bạn có thể nhập mục tiêu tập trung của mình (ví dụ: "Đọc 15 trang tài liệu") tối đa 100 ký tự. Nhấn Enter hoặc nhấp chuột ra ngoài để tự động lưu và hiển thị cho partner thấy.
    *   **Trò chuyện**: Sử dụng hộp Mini Chat ở cột bên phải để thảo luận, khích lệ và giao lưu trước phiên học.
    *   **Xác nhận bắt đầu phiên**: Cơ chế xác nhận 2 bước:
        1.  Bạn hoặc partner nhấn nút **"Bắt đầu phiên ngay"**.
        2.  Người còn lại sẽ nhận được yêu cầu xác nhận. Nếu họ đồng ý và nhấn **"Xác nhận"**, phiên sẽ bắt đầu.
    *   **Đang chờ xác nhận**: Nếu bạn đã bấm bắt đầu nhưng partner chưa bấm, UI sẽ hiển thị "Đang chờ partner xác nhận...". Bạn có thể bấm "Chờ thêm một chút" để hủy yêu cầu.
    *   **Cảnh báo chờ lâu (Timeout)**: Nếu partner chưa sẵn sàng sau 2 phút chờ đợi trong Lobby, hệ thống hiển thị cảnh báo giúp bạn chọn **Tiếp tục chờ** hoặc **Rời phòng**.
    *   **Đếm ngược bắt đầu**: Khi cả hai người cùng xác nhận, hệ thống bắt đầu đếm ngược đồng bộ 3-2-1 trước khi tự động kích hoạt Camera AI cục bộ và bộ đếm thời gian tập trung chính thức.
    *   **Rời/Hủy phòng**: Bạn có thể nhấn "Rời phòng" để thoát ra ngoài. Nếu là partner, hành động này đưa creator về trạng thái chờ partner mới; nếu là creator, hành động này sẽ giải tán hoàn toàn phòng.
*   **Chờ partner (Waiting Room):**
    *   Nếu là phòng Private, hệ thống sẽ cấp cho bạn một **Mã mời (Invite Code)** để chia sẻ cho bạn bè.
    *   Nếu là phòng Public, bạn chỉ cần đợi người dùng khác chọn tham gia từ danh sách chờ.
    *   Khi có người tham gia, phòng sẽ tự động chuyển sang giai đoạn **Ready Room** để hai bên cùng chuẩn bị.
    *   Bạn có thể nhấn **Hủy phòng** bất cứ lúc nào để giải tán phòng chờ.
*   **Phiên tập trung (Active Session):**
    *   **Cấp quyền Camera**: Bạn cần cấp quyền truy cập Camera. Momentum cam kết **không ghi hình, không gửi video lên server**. Toàn bộ quá trình nhận diện diễn ra 100% trên thiết bị của bạn.
    *   **AI Coach Real-time**: 
        - **Focused (Xanh)**: Khi AI nhìn thấy bạn đang ở trước màn hình.
        - **Away (Đỏ)**: Khi bạn rời khỏi khung hình quá 5 giây. Hệ thống sẽ rung cảnh báo và hiển thị thông điệp nhắc nhở quay lại.
    *   **Hệ thống Chỉ số**:
        - **Focus Score**: Điểm số tập trung thực tế, bị trừ điểm mỗi khi bạn rời đi.
        - **Presence**: Tỷ lệ thời gian bạn có mặt trong suốt phiên.
        - **Away Count**: Số lần bạn đã bị AI "bắt quả tang" khi rời vị trí.
    *   **Mini Chat & Partner Stats (Giao tiếp Real-time)**:
        - **Theo dõi chỉ số Partner**: Bạn có thể theo dõi trực tiếp mức độ tập trung (Focus), độ hiện diện (Presence) và số lần xao nhãng (Away) của người cùng phòng trên thanh bên phải (Partner Sidebar). Giao diện sẽ cảnh báo nhấp nháy đỏ khi bạn cùng phòng mất tập trung.
        - **Mini Chat**: Thanh Chat nhỏ gọn ở góc dưới thanh thông số hỗ trợ trò chuyện nhanh không làm che khuất camera chính. Hỗ trợ cuộn tự động và tính năng thu gọn thông minh (có huy hiệu đếm tin nhắn chưa đọc).
        - **Thông báo AI tự động**: Mỗi khi Partner đổi trạng thái quan trọng (rời khỏi bàn làm việc hoặc vừa quay trở lại), Chat Box sẽ chèn tự động một thông báo hệ thống màu vàng cam để giúp bạn dễ dàng bám sát tình hình mà không cần nhìn sang khung hình nhỏ.
*   **Báo cáo phiên (Session Report):**
    *   Sau khi kết thúc, bạn sẽ nhận được bảng tổng kết hiệu suất thật.
    *   **XP Earned**: Điểm kinh nghiệm được thưởng dựa trên Focus Score thật của bạn.
    *   **AI Insight**: Những lời khuyên cá nhân hóa từ AI Coach giúp bạn cải thiện kỷ luật.
